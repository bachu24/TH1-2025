...
"""
=================================================================
  Model 4: Bidirectional LSTM — Group-based Classification
=================================================================
...
def load_sequences():
        ...
        for s in sorted(os.listdir(gp)):
            ...
            if vecs:
                ...
                if mat.shape[0] < MAX_WIN:
                    ...
                    mat = mat[:MAX_WIN]
                subj_seqs[s] = mat; subj_labels[s] = g_label
    ...
    sel = SelectKBest(f_classif, k=K_FEAT); sel.fit(ams, all_y); mask = sel.get_support()
...
def main():
    ...
    print("=" * 60); print("  MODEL 4: Bi-LSTM"); print("=" * 60)
    X, y, subs = load_sequences()
    ...
    for fold_i, (tr, te) in enumerate(cv.split(range(len(subs)), y)):
        ...
        model = Sequential([
            ...
            Bidirectional(LSTM(32)), BatchNormalization(), Dropout(0.3),
        ...
        inv = {v: k for k, v in lm.items()}; ypo = [inv[p] for p in yp]
    ...
    ConfusionMatrixDisplay(cm, display_labels=[GROUP_NAMES[i] for i in labels]).plot(ax=ax, cmap='Purples')
    ax.set_title(f'Bi-LSTM CV (Acc={cv_acc:.1%})'); plt.tight_layout()
    plt.savefig(os.path.join(OUT, 'bilstm_cm.png'), dpi=150); plt.close()
    ...
    a1.set_title('Bi-LSTM Loss'); a1.set_xlabel('Epoch'); a1.legend()
    a2.plot(h.history['accuracy'], label='Train'); a2.plot(h.history['val_accuracy'], label='Val')
    a2.set_title('Bi-LSTM Accuracy'); a2.set_xlabel('Epoch'); a2.legend()
    plt.tight_layout(); plt.savefig(os.path.join(OUT, 'bilstm_curves.png'), dpi=150); plt.close()

    results = {'model': 'Bi-LSTM', 'cv5_accuracy': round(cv_acc, 4),
    ...
    with open(os.path.join(OUT, 'bilstm_results.json'), 'w') as f:
        json.dump(results, f, indent=2, default=str)
    print(f"\n  Saved: bilstm_results.json, bilstm_cm.png, bilstm_curves.png")
